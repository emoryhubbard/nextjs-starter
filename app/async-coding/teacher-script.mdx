...
React class components
    ...
    Mount/Unmount (for example for mount using api call for initial or placeholder data and for unmount removing event listeners so they don't accumulate)

    Update (for example api call will need componentDidUpdate e.g. Pokedex number updates automatically with no need for button click)
